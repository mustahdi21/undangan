...
document.getElementById('rsvpForm')?.addEventListener('submit',async(e)=>{e.preventDefault();const fd=new FormData(e.target);const r=await fetch('api/rsvp.php',{method:'POST',body:fd});document.getElementById('rsvpResult').textContent=(await r.json()).message;});
setInterval(async()=>{const r=await fetch('api/wishes.php');const data=await r.json();const box=document.getElementById('wishes');box.innerHTML=data.map(w=>`<div class='glass p-3 rounded-xl'><b>${w.guest_name}</b><p>${w.message}</p></div>`).join('')},8000);
