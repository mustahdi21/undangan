...
4. File `.htaccess` sudah disediakan pada:
   - `/public/.htaccess` (front controller routing jika DocumentRoot sudah ke `public`)
   - `/.htaccess` + `/index.php` (fallback front controller jika DocumentRoot masih di project root, tanpa redirect loop).
...
### 12) Solusi AH00124 (Request exceeded the limit of 10 internal redirects)
Error `AH00124` berarti terjadi rewrite loop.
Perbaikan yang dipakai di project ini:
- `/.htaccess` tidak lagi redirect ke `/public/...`
- `/.htaccess` sekarang rewrite langsung ke root `index.php`
